...
function Dashboard() {
  ...
  const handleAddPatient = async (patientData: Omit<Patient, 'id'>) => {
    ...
    try {
      ...
      setError('Patient added. Please refresh to see changes..');
  ...
  const stats = {
    ...
    upcomingAppointments: patients.filter(
      (p) => p.appointment && new Date(p.appointment) > new Date()
    ).length
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <StatsCard title="In Clinic Today" value={stats.inClinicToday} icon={Building2} color="green" />
          <StatsCard title="Upcoming Appointments" value={stats.upcomingAppointments} icon={Calendar} color="purple" />
        </div>
